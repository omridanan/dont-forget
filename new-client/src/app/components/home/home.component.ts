...
export class HomeComponent implements OnInit {
  ...
  saveNewNote() {
    let draft: any = {};
    draft._id = 'note_' + Math.floor(Date.now() / 1000);
    draft.color = "label-info";
    draft.time = new Date().toISOString();
    draft.label = "";
    draft.reminder = null;
    draft.restore = "note";
    draft.title = "Dentist";
    draft.note = "Go to dentist";
    draft.repeat = "yearly";

    this._notesService.saveNote('notes', draft)
        .then(res => {
          this.updateOrder(draft);
          this.inputFocusClass = false;
          draft = {};
          this.refreshNotesTables();
        }, err => {
